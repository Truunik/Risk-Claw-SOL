...
| FR-5 | `checkThresholdBreach` | Submits `risk_policy::queue_threshold_check` with `metrics.notionalUSD` as score **iff >5s since the last successful call for this policy** (matches the G1 onchain rate limit so we never hit a guaranteed-reject). Awaits `ThresholdCheckEvent` callback (max 30s) on submitted calls. | Must |
| FR-5b | `checkThresholdBreach` (throttle) | Maintains an in-memory per-policy `{lastCheckedAt, lastResult}` map inside `RealClient`. Within 5s of a successful call, returns the cached prior result without re-submitting. Cache clears on `RealClient` instantiation. **Resolves the tick-rate vs. rate-limit collision** — Builder A's Analyst fires `evaluate()` on every Helius LaserStream tick; this throttle means the boundary doesn't break under that load. | Must |
